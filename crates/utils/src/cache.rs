...
impl CacheService {
    ...
    // pub fn new(db_location: &str) -> sled::Result<Self> {
        ...
        V: Serialize,
    {
        ...
        let expiry = match duration_in_seconds {
            Some(duration) => Some(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() + duration),
            None => None,
        };
...
mod tests {
    ...
    fn test_create_db_connection() {
        ...
        cache.store_key_value("ping", "pong", None).expect("insert failed");
    ...
    fn test_new_cache_service() {
        ...
        // let cache_service = CacheService::new().unwrap();
