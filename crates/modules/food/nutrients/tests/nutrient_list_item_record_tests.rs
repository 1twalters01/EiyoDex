...
async fn test_database_operations() {
    ...
    let retrieved_nutrient_list_a_items = NutrientListItemRecord::load_all_from_sqlite(&nutrient_list_a_record.get_id(), &pool).await.unwrap();
    let retrieved_nutrient_list_b_items = NutrientListItemRecord::load_all_from_sqlite(&nutrient_list_b_record.get_id(), &pool).await.unwrap();
    let retrieved_nutrient_list_c_items = NutrientListItemRecord::load_all_from_sqlite(&nutrient_list_c_record.get_id(), &pool).await.unwrap();
    ...
    // delete from list
    nutrient_list_b_item_record_vec[0].delete_item_from_sqlite(&pool).await.unwrap();
    let retrieved_nutrient_list_b_items_2 = NutrientListItemRecord::load_all_from_sqlite(&nutrient_list_b_record.get_id(), &pool).await.unwrap();
    assert!(retrieved_nutrient_list_b_items_2.len() == retrieved_nutrient_list_b_items.len() - 1);

    refs = retrieved_nutrient_list_b_items_2.iter().collect();
    NutrientListItemRecord::delete_item_vec_from_sqlite(refs, &pool).await.unwrap();
    let retrieved_nutrient_list_b_items_3 = NutrientListItemRecord::load_all_from_sqlite(&nutrient_list_b_record.get_id(), &pool).await.unwrap();
